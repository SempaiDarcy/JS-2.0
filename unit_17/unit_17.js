...
function t5() {
    a5_res = a5.filter(elem=>{
        if (typeof(elem) !== "string" && elem%2!==1){
            return elem;
        }
    })
    return a5_res;
...
function t8() {
        ...
        let a8res = a8_res.filter((elem, index) => {
            if (index % 2 !== 0) {
                return true;
            }
        });
...
function t11() {
    a11_res = a11.map(function (elem,index){
        return index;
    })
    let a11res = a11.filter(function (elem,index){
        if (index%2===0){
            return true;
        }
    })
...
function t15() {
    a15_res = a15.filter(elem =>{
        if (elem.name !== '' && elem.pnum.length === 6){
            return elem;
        }
    })
    return a15_res;
