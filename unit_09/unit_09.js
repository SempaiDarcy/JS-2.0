...
function f7() {
    let p = 0;
    while (p<blocks7.length){
        blocks7[p].classList.add('bg-orange');
        p++;
    }
...
function f8() {
    let p = 0;
    while (p<blocks8.length){
        blocks8[p].classList.toggle('bg-orange');
        p++;
    }
...
function f18() {
    let get = document.querySelector('.p-18').getAttribute('data-b');
    document.querySelector('.out-18').innerHTML = get;
...
function f20() {
let a20 = document.querySelector('.out-20');
a20.setAttribute('title','go');
}
